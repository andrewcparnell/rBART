...
IntegerVector find_internal_nodes(NumericMatrix treetable){
  ...
  //std::cout<<"test to see if 1 is in internal_nodes"<<" "<<test<<"\n";
...
using namespace Rcpp;
// [[Rcpp::export]]
List append_parent_child(NumericMatrix treedata,IntegerVector internal_nodes){
  List parent_child(2);
  ...
  for(int i=0; i<internal_nodes.size();i++){
    bool elem_pres=Find_element(treedata(internal_nodes[i]-1,0),internal_nodes);
   // std::cout<<"find_node is"<<treedata(internal_nodes[i]-1,0)<<" "<<"elem_pres is "<<elem_pres<<"\n";
    ...
    if(elem_pres){
      ...
      swap_child.push_back(treedata(internal_nodes[i]-1,0));  
      //std::cout<<"swap_parent is "<<swap_parent[i]<<"\n";
      //std::cout<<"swap_child is "<<swap_child[i]<<"\n";
    }
     elem_pres=Find_element(treedata(internal_nodes[i]-1,1),internal_nodes);
     if(elem_pres){
        swap_parent.push_back(internal_nodes[i]);
        swap_child.push_back(treedata(internal_nodes[i]-1,1));  
      }
  }
...
NumericMatrix set_swap(NumericMatrix treedata,int swap_parent,int swap_child){
  double temp_sv;
  double temp_sp;
  temp_sv=treedata(swap_parent-1,2);
  temp_sp=treedata(swap_parent-1,3);
  //std::cout<<"Parent Var and point to change are"<<temp_sv<<" "<<temp_sp<<"\n";
  //treedata(swap_parent,3:4)<-treedata[swap_child,3:4]
  treedata(swap_parent-1,2)=treedata(swap_child-1,2);
  treedata(swap_parent-1,3)=treedata(swap_child-1,3);
  //treedata[swap_child,3:4]<-temp
  treedata(swap_child-1,2)=temp_sv;
  treedata(swap_child-1,3)=temp_sp;
  return(treedata);
}
...
List swap_nodes(NumericMatrix xmat,NumericMatrix treedata,IntegerMatrix tree_matrix){
  IntegerVector internal_nodes;
  internal_nodes=find_internal_nodes(treedata);
  
  List parent_child=append_parent_child(treedata,internal_nodes);
  ...
  if((treedata.nrow()==1) | (tree_matrix.ncol()==2)){
    swap_output[0]=treedata;
    swap_output[1]=tree_matrix;
  }else{
   ...
   //create set_swap() function
    set_swap(treedata,swap_parent,swap_child); 
    std::cout<<treedata(swap_child,2) <<"\n";
    //find left daughter and update tree_matrix and mean st dev if it is a terminal node
    update_tree_matrix(xmat,swap_parent,treedata,tree_matrix);
