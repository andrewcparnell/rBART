# This version of BART runs two-class classification
...
classBART = function(X, y, # X is the feature matrix, y is the target
  ...
  z_store = matrix(NA, ncol = length(y), nrow = store_size)
...
change_tree = function(X, y, curr_tree, node_min_size) {
  ...
  while(bad_trees) {
    ...
    } else if(any(new_tree$tree_matrix[internal_nodes, 'node_size'] < node_min_size)) {
      count_bad_trees = count_bad_trees + 1
    } else {
      bad_trees = FALSE
    }
...
swap_tree = function(X, y, curr_tree, node_min_size) {
  ...
  while(bad_trees) {
    ...
    # Check for bad tree
    # A bad tree might have an empty terminal node 
    # .. or a bad internal node which has less than node_min_size observations
    if(any(new_tree$tree_matrix[terminal_nodes, 'node_size'] == 0)) {
      count_bad_trees = count_bad_trees + 1
    } else if(any(new_tree$tree_matrix[internal_nodes, 'node_size'] < node_min_size)) {
      count_bad_trees = count_bad_trees + 1
...
predict_classBART = function(newX, classBART_posterior, 
  ...
  for (i in 1:n_its) {
    ...
    curr_trees = classBART_posterior$trees[[i]]
...
plot_tree = function(classBART_posterior, 
                     iter = sample(1:classBART_posterior$store_size, 1), 
                     tree_num = sample(1:classBART_posterior$num_trees, 1),
                     horiz = TRUE) {
  ...
  curr_tree = classBART_posterior$trees[[iter]][[tree_num]]$tree_matrix
...
classBART_CV = function(X, y, folds = 5, num_trees = 2, ...) {
  # Run classBART with k-fold cross validation
  fold_id = sample(1:folds, size = length(y), replace=TRUE)
  
  # Create holder for predictions
  oob_predictions = rep(NA, length(y))
  # Holder for trees
  all_classBART_runs = vector('list', folds)
  ...
  for (i in 1:folds) {
    ...
    all_classBART_runs[[i]] = classBART(X_in, y_in, 
                                num_trees = num_trees, ...)
    oob_predictions[fold_id == i] = predict_classBART(newX = X_out, 
                                                  all_classBART_runs[[i]],
                                                  type = 'mean')
  }

  # Return output
  return(list(all_classBART_runs = all_classBART_runs,
