...
tree_full_conditional = function(tree, R, a, nu, lambda) {
  ...
  SS_Rj = aggregate(R, by = list(tree$node_indices), 
                    function(x) sum((x-mean(x))^2))[,2]
...
rBART2_CV = function(X, y, folds = 5, num_trees = 2, ...) {
  ...
  for (i in 1:folds) {
    ...
    all_rBART2_runs[[i]] = rBART2(X_in, y_in, 
                                num_trees = num_trees, ...)
    oob_predictions[fold_id == i] = predict_rBART2(newX = X_out, 
                                                  all_rBART2_runs[[i]],
                                                  type = 'mean')
  }

  # Return output
  return(list(all_rBART2_runs = all_rBART2_runs,
