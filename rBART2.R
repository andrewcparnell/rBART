# This is a version of rBART with varying sigma at each terminal node
# Do not start with this version, start with rBART

# Main function -----------------------------------------------------------

rBART2 <- function(X, y, # X is the feature matrix, y is the target
  ...
  a <- priors$a # Precision for overall mean
  ...
  predictions <- get_predictions(curr_trees, 
                                 X, 
...
create_stump <- function(num_trees,
  ...
  # Each tree has 9 columns and 2 elements
  ...
  for (j in 1:num_trees) {
    ...
    all_trees[[j]][[1]] <- matrix(NA, ncol = 9, nrow = 1)
    ...
    all_trees[[j]][[1]][1, ] <- c(1, 1, NA, NA, NA, NA, 0, 1, length(y))
...
grow_tree <- function(X, y, curr_tree, node_min_size) {
  ...
  split_value <- sample(available_values[-c(1:2, length(available_values))], 1)
...
change_tree <- function(X, y, curr_tree) {
  ...
  while (bad_trees) {
    ...
    # Check for bad tree
    if (any(new_tree$tree_matrix[terminal_nodes, "node_size"] < 2)) {
...
tree_full_conditional <- function(tree, R, a, nu, lambda) {
...
get_predictions <- function(trees, X, single_tree = FALSE,
  ...
  if (single_tree) {
    ...
    if (nrow(trees$tree_matrix) == 1) {
      predictions <- rep(trees$tree_matrix[1, type], nrow(X))
    } else {
      ...
      for (i in 1:length(unique_node_indices)) {
        predictions[curr_X_node_indices == unique_node_indices[i]] <-
          trees$tree_matrix[unique_node_indices[i], type]
      }
    }
    ...
    predictions <- get_predictions(trees[[1]], X, 
                                   single_tree = TRUE, 
                                   type = type) +
      get_predictions(partial_trees, X,
...
simulate_mu_tau <- function(tree, R, a, nu, lambda) {
...
predict_rBART2 <- function(newX, rBART2_posterior,
  ...
  type <- match.arg(type, several.ok = FALSE)
  par <- match.arg(par, several.ok = FALSE)

  # Create holder for predicted values
  n_its <- nrow(rBART2_posterior$y_hat)
  y_hat_mat <- matrix(NA,
    nrow = n_its,
    ncol = nrow(newX)
  )
  ...
  for (i in 1:n_its) {
    ...
    curr_trees <- rBART2_posterior$trees[[i]]
...
plot_tree <- function(rBART2_posterior,
                      iter = sample(1:rBART2_posterior$store_size, 1),
                      tree_num = sample(1:rBART2_posterior$num_trees, 1),
                      horiz = TRUE) {
  ...
  curr_tree <- rBART2_posterior$trees[[iter]][[tree_num]]$tree_matrix
  ...
  node_number_swap <- cbind(
    c(terminal_nodes, internal_nodes),
    1:nrow(curr_tree)
  )
...
rBART2_CV <- function(X, y, folds = 5, num_trees = 2, ...) {
  # Run rBART2 with k-fold cross validation
  fold_id <- sample(1:folds, size = length(y), replace = TRUE)

  # Create holder for predictions
  oob_predictions <- rep(NA, length(y))
  # Holder for trees
  all_rBART2_runs <- vector("list", folds)
  ...
  for (i in 1:folds) {
    ...
    all_rBART2_runs[[i]] <- rBART2(X_in, y_in,
      num_trees = num_trees, ...
    )
    oob_predictions[fold_id == i] <- predict_rBART2(
      newX = X_out,
      all_rBART2_runs[[i]],
      type = "mean"
    )
  ...
  # Return output
  return(list(
    all_rBART2_runs = all_rBART2_runs,
